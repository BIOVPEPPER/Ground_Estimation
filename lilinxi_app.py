...
class pointCloud_Request(BaseModel):
    URL: str


@lru_cache(500, typed=True)
...
@app.post("/Ground_Estimation")
async def predict_points(data: pointCloud_Request):
    print("来了新请求")
    corner_List, predict_Z = test_with_Ransac_with_URL(data.URL, max_iter=1000)
    all_Points = []
    for i in range(len(predict_Z)):
        all_Points.append({'x': int(corner_List[i][0]), 'y': int(corner_List[i][1]), 'z': float(predict_Z[i])})
    return all_Points
...
if __name__ == '__main__':
    uvicorn.run(
        "lilinxi_app:app",
        # GPU2 公司公网8000-9100端口是对外开放的，GPU1不开放
        host="0.0.0.0", port=9000,
        log_level="info",
        debug=False,
        # ssl_keyfile="example.com+5-key.pem",
        # ssl_certfile="example.com+5.pem"
        # workers=1
    )
